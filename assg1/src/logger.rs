...
pub struct Discoverer<P: Problem> {
    best_solution: Option<(P::Solution, P::Measure)>,
}

impl<P: Problem> Log<(P::Solution, P::Measure)> for Discoverer<P> {
    fn log(&mut self, value: &(P::Solution, P::Measure)) {
        use crate::problem::BetterThan;

        let (_, measure) = value;
        match &self.best_solution {
            Some((_, best_measure)) if best_measure.better_than(measure) => (),
            _ => self.best_solution = Some(value.to_owned())
        }
    }
}

impl<P: Problem> Discoverer<P> {
    pub fn new() -> Discoverer<P> {
        Discoverer {
            best_solution: None,
        }
    }

    pub fn get_best_solution(&self) -> Option<P::Solution> {
        return self.best_solution.to_owned().map(|(solution, _)| solution);
    }

    pub fn get_best_fitness(&self) -> Option<P::Measure> {
